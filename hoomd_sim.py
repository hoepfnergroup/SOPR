def mie_fluid(it, m, spacing, name, timestep, n, sigma, epsilon, kbT, density, eq_time, production_time):
    ...
    with gsd.hoomd.open(name='traj/lattice' + str(it) +'.gsd', mode='w') as f:
        f.append(snapshot)

    # Pair hoomd to the cpu and create basic sim objects
    cpu = hoomd.device.CPU()
    sim = hoomd.Simulation(device=cpu, seed=1)
    sim.create_state_from_gsd(filename='traj/lattice' + str(it) +'.gsd')
    ...
    gsd_writer = hoomd.write.GSD(filename='traj/mie' + str(it) +'.gsd',
...
def table_fluid(it, m, spacing, name, timestep, kbT, density, eq_time, production_time, V, F, rmin, rcut):
    ...
    gsd_writer = hoomd.write.GSD(filename='traj/traj' + str(it) +'.gsd',
                              trigger=hoomd.trigger.Periodic(1000),
                              mode='wb')
    sim.operations.writers.append(gsd_writer)
    thermodynamic_properties = hoomd.md.compute.ThermodynamicQuantities(filter=hoomd.filter.All())
